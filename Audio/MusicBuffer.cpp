...
void MusicBuffer::Pause() const {
	alSourcePause(p_Source);
	AL_CheckAndThrow();
}

void MusicBuffer::Stop() const {
	alSourceStop(p_Source);
	AL_CheckAndThrow();
}

void MusicBuffer::Resume() const {
	alSourcePlay(p_Source);
	AL_CheckAndThrow();
}

void MusicBuffer::UpdateBufferStream() const {
...
ALint MusicBuffer::GetSource() const {
	return p_Source;
}
...
void MusicBuffer::SetGain(float val) const {
	if (val < 0)
		val = 0.0f;
